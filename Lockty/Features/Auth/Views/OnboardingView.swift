...
struct OnboardingView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // MARK: - Top overlay: back button + progress bar
            VStack(spacing: 0) {
            ...
            // MARK: - Bottom overlay: CTA button + sección secundaria
            VStack(spacing: BaseTheme.Spacing.xs) {
                ...
                // Sección secundaria del overlay — inyectada por cada step via PreferenceKey
                HStack {
    ...
    // MARK: - Step content con transición
    ...
    private var stepContent: some View {
        ZStack {
            ...
            if vm.currentStep == .permissions {
                PermissionsStep(vm: vm)
                    .geometryGroup()
                    .transition(AnyTransition(.blurReplace))
            }
            if vm.currentStep == .signIn {
                SignInStep(vm: vm, router: router)
                    .geometryGroup()
                    .transition(AnyTransition(.blurReplace))
            }
        }
        // .transition y .animation gestionados con withAnimation(animation) en las acciones
    }
